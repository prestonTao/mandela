...
func timingFirst() {
	FD.FileChunkInfo.Range(func(k, v interface{}) bool {
		...
		finfo, err := ParseFileinfo(chunkdata.FInfo)
...
//根据文件块获取fileinfo
func (fd *FileData) GetFileInfo(chunkid *nodeStore.AddressNet) *FileInfo {
...
type FileInfoData struct {
	FHash *nodeStore.AddressNet //文件hash
	CHash *nodeStore.AddressNet //块hash
...
func (cid *ChunkInfoData) MergeFileUser(fold, fnew []byte) []byte {
	f1, err := ParseFileinfo(fold)
	if err != nil {
		fmt.Println(err)
		return nil
	}
	f2, err := ParseFileinfo(fnew)
	if err != nil {
		fmt.Println(err)
		return nil
	}
