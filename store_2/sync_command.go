...
//广播数据消息
func sendData(id *nodeStore.AddressNet, data *SyncData) error {
	//如果发给自己,则特殊处理
	...
	if message, ok := mc.SendSearchAllMsg(config.MSGID_store_syncFileInfo, id, &content); ok {
...
//加入共享块用户为文件共享者
func addFileinfoToMyself(shareid *nodeStore.AddressNet, fid *FileInfoData) {
...
//加入1/4节点为块的默认共享用户
func addQuarterUser(fi *FileInfo, chunkhash *nodeStore.AddressNet) *FileInfo {
