...
/*
	查询已经保存的文件片总数量
*/
func (this *Storage) FindUseTotal() (uint64, error) {
	total := 0
	for i := uint64(0); i < this.TableNum; i++ {
		sqlStr := "select count(*) as count from key" + strconv.Itoa(int(i)) + " where status=?"
		result, err := this.sqldb.Query(sqlStr, 2)
		if err != nil {
			fmt.Println(err)
			return 0, err
		}
		...
		var totalOne uint64
		binary.Read(bytesBuffer, binary.LittleEndian, &totalOne)
		total = total + int(totalOne)
	}
	return uint64(total), nil
}

func (this *Storage) FullTable(tags chan bool) {
...
func NewStorage(vid virtual_node.AddressNetExtend, abspath string, index uint64) (*Storage, error) {
