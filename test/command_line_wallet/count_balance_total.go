...
const (
	...
	// addrOne = "TESTKLRCKRLDzBFXJUxT9LJm8VaSBaZmirVDo4"
	addrOne = "TEST3E1LDivNFVBrqoPMcb3CXM291ikAuPoYJ4"
)
		...
		for _, txBase := range bhvo.Txs {
			...
			if txBase.Class() == config.Wallet_tx_type_mining {
				...
				for vinIndex, one := range *txBase.GetVin() {
					...
					// if bytes.Equal(addrSpecial, vout.Address) {
					// 	engine.Log.Info("使用可用余额:%s %d %s %d", hex.EncodeToString(one.Txid), one.Vout, vout.Address.B58String(), vout.Value)
					// 	_, ok := txItemKey[utils.Bytes2string(one.Txid)+strconv.Itoa(int(one.Vout))]
					// 	if !ok {
					// 		// engine.Log.Info("未找到这个可用余额:%s %d", hex.EncodeToString(one.Txid), one.Vout)
					// 		engine.Log.Info("未找到这个可用余额:%s %d %s %d", hex.EncodeToString(one.Txid), one.Vout, vout.Address.B58String(), vout.Value)
					// 		panic("")
					// 	}

					// 	newNotSpendValue := uint64(0)
					// 	newFrozenValue := uint64(0)
					// 	if oldNotSpendValue > vout.Value {
					// 		newNotSpendValue = oldNotSpendValue - vout.Value
					// 		newFrozenValue = oldFrozenValue
					// 	} else {
					// 		newNotSpendValue = oldNotSpendValue
					// 		newFrozenValue = oldFrozenValue - vout.Value
					// 	}
					// 	engine.Log.Info("特殊地址:%s 原可用:%d 原锁定:%d -%d 新可用:%d 新锁定:%d", vout.Address.B58String(), oldNotSpendValue, oldFrozenValue, vout.Value, newNotSpendValue, newFrozenValue)
					// }
					...
					if ok {
						if oldNotSpendValue < vout.Value {
							...
							if !fok || oldFrozenValue < vout.Value {
								engine.Log.Info("交易hash:%s vinIndex:%d addr:%s oldValue:%d value:%d", hex.EncodeToString(*txBase.GetHash()),
			...
			for voutIndex, _ := range *txBase.GetVout() {
				...
				// if bhvo.BH.Height == 599798 {
				// 	engine.Log.Info("特定高度可用余额:%d %s %d", voutIndex, one.Address.B58String(), one.Value)
				// }
				// have := false
				// if bytes.Equal(addrSpecial, one.Address) {
				// 	engine.Log.Info("创建可用余额:%s %d %d", hex.EncodeToString(*txBase.GetHash()), voutIndex, one.Value)
				// 	key := utils.Bytes2string(*txBase.GetHash()) + strconv.Itoa(voutIndex)
				// 	txItemKey[key] = 0
				// 	// have = true
				// 	// engine.Log.Info("特殊地址:%s +%d", one.Address.B58String(), one.Value)
				// }
