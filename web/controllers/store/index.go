...
func (this *Index) GetList() {
	_, fileInfos := store.GetFileindexToSelfAll()
...
func (this *Index) AddFile() {
	...
	//文件切片
	fi, err := store.Diced(filepath.Join(config.Store_temp, filename))
	fi.Name = filename
	if err != nil {
	...
	//保存文件索引到本地
	err = store.AddFileindexToSelf(fi, vnodeinfo.Vid)
	if err != nil {
	...
	//这个文件索引归自己管理
	store.AddFileindexToNet(fi, vnodeinfo.Vid)
...
func (this *Index) AddCryptFile() {
	...
	//加密后的文件名
	cfilename := filename + store.Suffix
	...
	cryptfile.Close()
	//文件切片
	fi, err := store.Diced(filepath.Join(config.Store_fileinfo_cache, cfilename))
...
func (this *Index) GetFile() {
	...
	haveLocal := true
	//查询文件信息
	fileinfo := store.FindFileindex(fid)
	if fileinfo == nil {
		haveLocal = false
		var err error
		fileinfo, err = store.FindFileindexOpt(fn)
	...
	//把文件下载到本地
	err := store.DownloadFileOpt(fileinfo, isDown)
	if err != nil {
	...
	if !haveLocal {
		...
		store.AddFileindexToLocal(fileinfo, vnodes[0].Vid)
	}
	...
	if fileinfo.CryptUser != nil && fileinfo.CryptUser.B58String() == nodeStore.NodeSelf.IdInfo.Id.B58String() {
		...
		dc, err := store.Decrypt(data)
		if err != nil {
			fmt.Println("解密文件失败")
			return
		}
		//解密后的文件名
		filename = strings.TrimRight(filename, store.Suffix)
