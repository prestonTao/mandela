...
type FileIndex struct {
	...
	FileOwner  map[string]FileOwner           //文件所有者 key 用户nid value FileOwner
 ...
 */
func (this *FileIndex) AddFileOwner(vnode virtual_node.Vnodeinfo) error {
	user := FileOwner{Vnodeinfo: vnode, UpdateTime: time.Now().Unix()}
	this.FileOwner[vnode.Nid.B58String()] = user
	return nil
}
...
func NewFileIndex(hash *virtual_node.AddressNetExtend, filename string, chunkCount uint64) *FileIndex {
	return &FileIndex{
		...
		FileOwner:  make(map[string]FileOwner),
