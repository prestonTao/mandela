...
func NewDownProc(fi *FileIndex) *DownProc {
	dp := DownProc{
...
func DownloadProgressOne(fi *FileIndex) (dp DProgress) {
	if fi == nil {
		return
	}
	db := sql.Downprogress{}
...
func DownloadProgress() (dps []DProgress) {
	...
	for _, row := range rows {
		...
		// 	continue
		// }
		fi, err := ParseFileindex(row.FileInfo)
		if err != nil {
			fmt.Println(err)
		}
		dp.Hash = fi.Hash.B58String()
